...
- `Selection range` — Optional character range for comment anchoring
...
| Instruction | Description |
|-------------|-------------|
| `Insert row after row {N}` | Insert row after row N |
| `Delete row {N}` | Delete row N |
| `Merge columns {X}-{Y} in row {N}` | Merge columns X through Y in row N |
| `Merge {X} cells in row {N}` | Merge X cells in row N |
...
Replace "text" with "text" (chars 20-25)
```
...
```
Ambiguity error: "the" appears 3 times in paragraph:
  chars 15-18
  chars 42-45
  chars 78-81
Please add position info: (chars N-M)
```
...
| `Paragraph not found` | Invalid paragraph number | Check paragraph numbering with `list_paragraphs.py` |
| `Format parse failed` | Invalid format keyword | Use keywords from the Format Edits table |
| `Table index out of range` | Table/row index out of range | Verify table structure with `list_paragraphs.py` |
| `Missing required field` | Missing header field | Add required fields: author, source, output |
